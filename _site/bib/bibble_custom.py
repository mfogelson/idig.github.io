...
def _mla_fmt(entry):
    """Format an MLA citation for an entry."""
    f = entry.fields
    authors = _author_list_rev(entry.persons['author'])
    title = f['title']
    venue = _venue(entry)
    date = _mla_date(entry)
    return '{0}. "{1}." {2}, {3}.'.format(authors, title, venue, date)

def _author_fmt_rev(author):
    """Format an author's full name."""
    return u' '.join(author.last() + author.first() + author.middle())
...
def _author_list_rev(authors):
    """Format a list of authors."""
    return _andlist(list(map(_author_fmt_rev, authors)))
